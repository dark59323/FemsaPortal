...
type MenuItem = {
  label: string;
  route: string;
  icon: Exclude<IconKey, 'menu' | 'user' | 'chevron'>;
};
    ...
    <div class="min-h-dvh grid grid-rows-[56px_1fr] bg-[#f1f2f3] text-gray-900">
      ...
      >
        ...
        <div class="ml-auto flex items-center gap-3">
          <button
            class="hidden md:inline-flex items-center gap-2 px-3 h-9 rounded-xl hover:bg-gray-100"
          >
            <span class="size-5" [innerHTML]="icon('bell')"></span>
            <span class="text-sm">Notificaciones</span>
          </button>
          <div
            class="size-9 md:size-10 rounded-full bg-brand-700 text-white grid place-items-center font-bold"
          >
            D
          </div>
        </div>
            ...
            <div class="py-2">
              <nav class="mt-1 text-sm">
                ...
                >
                  ...
                  <span
                    class="uppercase tracking-wide font-extrabold text-[15px] opacity-90 whitespace-nowrap"
                    *ngIf="!collapsed()"
                    >{{ m.label }}</span
                  >
                  <span
        ...
        <main class="p-6 md:p-10 overflow-auto">
          <section class="rounded-2xl bg-white shadow-sm border p-8 md:p-12">
            <h1 class="text-4xl md:text-6xl font-extrabold leading-tight">
              <span
                class="bg-clip-text text-transparent bg-gradient-to-r from-gray-900 to-gray-600"
              >
                Bienvenido Danny al<br class="hidden md:block" />
                portal de FEMSA
              </span>
            </h1>
...
export class HomeComponent {
  ...
  icon(kind: IconKey): string {
    ...
    const wrap = (d: string, vb = '0 0 24 24') =>
      `<svg viewBox="${vb}" ${base} xmlns="http://www.w3.org/2000/svg">${d}</svg>`;
    switch (kind) {
      case 'menu':
        return wrap('<path d="M4 6h16M4 12h16M4 18h16"/>');
      case 'globe':
        return wrap(
          '<circle cx="12" cy="12" r="10"/><path d="M2 12h20"/><path d="M12 2a15 15 0 0 1 0 20a15 15 0 0 1 0-20z"/>'
        );
      case 'mail':
        return wrap('<rect x="3" y="5" width="18" height="14" rx="2"/><path d="m3 7 9 6 9-6"/>');
      case 'bell':
        return wrap(
          '<path d="M6 8a6 6 0 1 1 12 0c0 7 3 5 3 8H3c0-3 3-1 3-8"/><path d="M10 21h4"/>'
        );
      case 'settings':
        return wrap(
          '<circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 1 1-4 0v-.09a1.65 1.65 0 0 0-1-1.51 1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.6 15 1.65 1.65 0 0 0 3.09 14H3a2 2 0 1 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06A2 2 0 1 1 7.04 4.3l.06.06A1.65 1.65 0 0 0 8.92 4h.09a1.65 1.65 0 0 0 1-1.51V2a2 2 0 1 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9c.37.5.6 1.12.6 1.8s-.23 1.3-.6 1.8z"/>'
        );
      case 'share':
        return wrap(
          '<circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><path d="M8.59 13.51l6.83 3.98M15.41 6.51 8.59 10.49"/>'
        );
      case 'user':
        return wrap('<path d="M20 21a8 8 0 1 0-16 0"/><circle cx="12" cy="7" r="4"/>');
      case 'chevron':
        return wrap('<path d="m15 18-6-6 6-6"/>');
    }
  }
}
